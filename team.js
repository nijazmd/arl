...
async function loadData() {
  ...
  const teamPointsMap = {};
  const teamDriverPoints = {};
  const driverRaceStats = {};

  resultLines.forEach(row => {
    const cols = row.split(",").map(s => s.replace(/"/g, "").trim());
    const [round, date, driver, team, car, track, level, chances, positionStr, pointsStr] = cols;
    ...
    if (!driverRaceStats[driver]) {
      driverRaceStats[driver] = { totalPoints: 0, races: 0, firsts: 0, podiums: 0 };
    }
  ...
  <div class="driver-card">
    ...
    <div class="driver-pair">
      <div class="pair">
        ...
        <div class="value">${driver.races}</div>
      </div>
      <div class="pair">
        <div class="label">1st Place</div>
        <div class="value">${driver.firsts}</div>
      </div>
      <div class="pair">
        <div class="label">Podiums</div>
        <div class="value">${driver.podiums}</div>
      </div>
      <div class="pair">
        <div class="label">Points</div>
        <div class="value">${driver.totalPoints}</div>
      </div>
      <div class="pair">
        <div class="label">Avg Points</div>
        <div class="value">${driver.avgPoints}</div>
      </div>
      <div class="pair">
        <div class="label">1st %</div>
        <div class="value">${driver.firstPct}%</div>
      </div>
      <div class="pair">
        <div class="label">Podium %</div>
        <div class="value">${driver.podiumPct}%</div>
      </div>
    </div>
  </div>
`).join("");
